...
def _reliability_status_class(status):
    return {"HEALTHY":"pos","DEGRADED":"warnc","QUARANTINED":"neg"}.get(status,"dim")

def symbol_reliability_overview_html(R):
    """Read-only presentation of the API-owned Symbol Reliability V1 snapshot.

    The cockpit does not calculate, persist, or feed this circuit breaker. It deliberately
    renders the executor API response so Dashboard cannot claim a quarantine that runtime did
    not actually consume.
    """
    o=["<h2>Symbol Reliability V1 · Circuit breaker</h2>",
       "<div class='note'>Actual-fill evidence only: completed FILTERED 3L/3S baskets with frozen NoTP + Hold36h policy, HORIZON close, complete accounting, and overlap-collapsed independent episodes. <b>Only QUARANTINED symbol+side is removed.</b> HEALTHY, DEGRADED, and INSUFFICIENT_DATA preserve Plain MOM36 ranking, score, weights, and exits. <b>DEGRADED = WARNING ONLY · DOES NOT CHANGE SELECTION.</b></div>"]
    for key in ("live","testnet"):
        info=R["inst"][key]; ex=info.get("ex") or {}; snapshot=ex.get("symbolReliability")
        o.append("<h3>%s</h3>"%esc(info.get("long") or key))
        ...
        if not statuses:
            ...
            o.append("<div class='scroll'><table><tr><th>symbol</th><th>side</th><th>status</th><th>score</th><th>N 90D</th><th>mean</th><th>PF</th><th>CVaR5</th><th>winner→loser</th><th>trend</th><th>reason</th></tr>")
            for row in sorted(statuses,key=lambda x:(str(x.get("symbol") or ""),str(x.get("side") or ""))):
                status=str(row.get("status") or "INSUFFICIENT_DATA")
                status_label="DEGRADED · WARNING ONLY · DOES NOT CHANGE SELECTION" if status=="DEGRADED" else status
                mean_return=row.get("meanContribution")
                cvar=row.get("cvar5")
                damage=row.get("winnerToLoserDamageRate")
                o.append("<tr><td><b>%s</b></td><td>%s</td><td><span class='%s'><b>%s</b></span></td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td style='white-space:normal'>%s</td></tr>"%(
                    esc(row.get("symbol") or "–"),esc(row.get("side") or "–"),_reliability_status_class(status),esc(status_label),
                    num(row.get("diagnosticScore"),0),row.get("independentN") or 0,
                    pct(100*mean_return,3) if fin(mean_return) else "–",num(row.get("profitFactor"),2),
                    pct(100*cvar,3) if fin(cvar) else "–",pct(100*damage,1) if fin(damage) else "–",
                    esc(row.get("trend") or "–"),esc(row.get("reason") or "–")))
            o.append("</table></div>")
        decision=snapshot.get("lastFormationDecision") or {}
        if isinstance(decision,dict) and decision:
            before=decision.get("selectedBefore") or {}; after=decision.get("selectedAfter") or {}
            replacements=decision.get("replacements") or []
            rep=" · ".join("%s %s→%s"%(str(x.get("side") or "?"),str(x.get("removed") or "?"),str(x.get("replacement") or "NO_TRADE")) for x in replacements) or "none"
            o.append("<div class='card' style='margin-top:9px'><div class='k'>Latest formation decision · %s</div><div style='margin-top:6px'><b>%s</b> · source <code>%s</code></div><div class='s'>LONG %s → %s · SHORT %s → %s · scoreGap %s → %s (floor %s)</div><div class='s'>replacement: %s</div><div class='note'>Candidate rows and rejection reason are frozen with the source observation; this display is audit-only.</div></div>"%(
                esc(decision.get("evaluatedAt") or "–"),esc(decision.get("decision") or "–"),esc(decision.get("sourceObservationId") or "–"),
                esc(", ".join(before.get("LONG") or []) or "–"),esc(", ".join(after.get("LONG") or []) or "–"),
                esc(", ".join(before.get("SHORT") or []) or "–"),esc(", ".join(after.get("SHORT") or []) or "–"),
                num(decision.get("scoreGapBefore"),4),num(decision.get("scoreGapAfter"),4),num(decision.get("scoreGapFloor"),4),esc(rep)))
        excluded=snapshot.get("excludedBaskets") or {}
        if excluded:
            o.append("<div class='note'>Excluded evidence: %s</div>"%esc(", ".join("%s=%s"%(k,v) for k,v in sorted(excluded.items()))))
    return "".join(o)
...
def futures_reference_health_html(R,detail=False):
    ...
    o.append(card("USD-M Mark Used",_fh_pair(R,"usdMMarkUsed"),"referensi sizing premiumIndex sejak proses mulai"))
    o.append(card("Book Fallback",_fh_pair(R,"bookFallback"),"fallback sizing USD-M midpoint saja"))
    ...
    if rows:
        ...
        o.append("</table></div>")
    o.append("<div class='note'>Counter sejak proses API mulai dan hanya mencatat sizing runtime. Probe dashboard hanya GET public USD-M exchangeInfo / mark / book untuk diagnosis per-symbol; ia tidak mengubah order, posisi, universe, sizing, atau counter cache.</div>")
    return "".join(o)
...
def tab_overview(R):
    ...
    if blocking: st,c,r="block","%d masalah menghalangi"%len(blocking),"%s — %s"%(blocking[0][2],blocking[0][3])
        ...
        stop="OFF" if liv["ex"].get("stopNetReturnPct") is None else "%s%%"%liv["ex"].get("stopNetReturnPct")
        tp="OFF" if liv["ex"].get("tpDisabled") else "%s%%"%liv["ex"].get("tpNetReturnPct")
        st,c,r="ok","Memegang %d basket di uang nyata"%len(ob_l),"Berjalan dengan batas tahan %s jam; stop %s · TP %s."%(liv["ex"].get("maxHoldHours"),stop,tp)
...
def tab_system(R):
    ...
    o.append("<details class='observability'><summary>Symbol Reliability V1 · circuit breaker</summary><div class='body'>%s</div></details>"%symbol_reliability_overview_html(R))
