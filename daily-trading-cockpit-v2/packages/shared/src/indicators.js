...
export function roundPrice(value, reference = value) {
...
function latestComparableVolume(values) {
    if (values.length < 2) {
...
function detectTrend(close, ema20Value, ema50Value, ema200Value) {
...
export function calculateTimeframeIndicators(candles, timeframe, now = Date.now()) {
    ...
    const volumeRatio = comparableVolume.current !== null && comparableVolume.baseline !== null && comparableVolume.baseline > 0
...
export function buildAtrPlan(price, atrValue, atrPercent, direction, fib) {
