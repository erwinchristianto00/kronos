...
export interface EffectiveState<T extends string | boolean> { symbol: string; effectiveTimeMs: number; value: T; sourceHash: string; }
...
export class EffectiveStateTimeline<T extends string | boolean> {
  ...
  constructor(rows: readonly EffectiveState<T>[]) {
    for (const row of rows) {
      if (!row.symbol || !row.sourceHash || !Number.isInteger(row.effectiveTimeMs) || row.effectiveTimeMs < 0) throw new Error("FOUNDRY_TIMELINE_ROW_INVALID");
...
export function listingTimeline(rows: readonly ValidatedFoundryRow[]): EffectiveStateTimeline<"LISTED" | "DELISTED"> {
  return new EffectiveStateTimeline(rows.map((row) => ({ symbol: row.symbol!, effectiveTimeMs: row.timestampMs, value: row.status as "LISTED" | "DELISTED", sourceHash: row.sourceHash })));
}

export function futuresTimeline(rows: readonly ValidatedFoundryRow[]): EffectiveStateTimeline<boolean> {
  return new EffectiveStateTimeline(rows.map((row) => ({ symbol: row.symbol!, effectiveTimeMs: row.timestampMs, value: row.available as boolean, sourceHash: row.sourceHash })));
}
