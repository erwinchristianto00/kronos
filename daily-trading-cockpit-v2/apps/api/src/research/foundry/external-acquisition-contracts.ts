...
export interface ExternalAcquisitionContract { artifactKind: FoundryArtifactKind; provider: string; dataset: string; schemaVersion: "v1" | "v2"; requiredFields: string[]; range: { startMs: number; endMs: number; symbols: string[] }; retrievalCommand: string; }
...
const requirements: Array<Omit<ExternalAcquisitionContract, "range">> = [
  ...
  { artifactKind: "LISTING_DELISTING_TIMELINE", provider: "Exchange historical export", dataset: "symbol listing and delisting state intervals", schemaVersion: "v2", requiredFields: ["symbol", "effectiveTimeMs", "validUntilMs", "status", "sourceHash"], retrievalCommand: "Export authoritative historical listing/delisting intervals through range end, or an exhaustive timestamped lifecycle crawl with a source-backed validity watermark; do not query current exchange state" },
  { artifactKind: "FUTURES_AVAILABILITY_TIMELINE", provider: "Exchange historical export", dataset: "USD-M contract availability state intervals", schemaVersion: "v2", requiredFields: ["symbol", "effectiveTimeMs", "validUntilMs", "available", "sourceHash"], retrievalCommand: "Export authoritative USD-M availability intervals through range end, or an exhaustive timestamped lifecycle crawl with a source-backed validity watermark; do not infer from candles" },
