...
const requirements: Array<Omit<ExternalAcquisitionContract, "range">> = [
  ...
  { artifactKind: "LISTING_DELISTING_TIMELINE", provider: "Exchange historical export", dataset: "symbol listing and delisting events", schemaVersion: "v1", requiredFields: ["symbol", "effectiveTimeMs", "status", "sourceHash"], retrievalCommand: "Export authoritative historical listing/delisting events; do not query current exchange state" },
  { artifactKind: "FUTURES_AVAILABILITY_TIMELINE", provider: "Exchange historical export", dataset: "USD-M contract availability events", schemaVersion: "v1", requiredFields: ["symbol", "effectiveTimeMs", "available", "sourceHash"], retrievalCommand: "Export authoritative USD-M availability history; do not infer from candles" },
  ...
  { artifactKind: "CANONICAL_EPISODES", provider: "Kronos canonical ledger export", dataset: "market-cause map", schemaVersion: "v1", requiredFields: ["symbol", "decisionTimeMs", "marketEpisodeId", "sourceHash", "canonicalAlgorithm"], retrievalCommand: "Export persisted canonical market-cause inputs or a complete symbol+decision-time canonical map" },
];
