...
const BOOK_TICKER_HOURLY_POLICY_VERSION = "binance-vision-usdm-bookticker-hourly-v4";
...
async function collectBookTickerSamples(input: { root: string; expectedCoverage: FoundryExpectedCoverage }): Promise<{ archiveBundle: ArchiveBundleIdentity; samples: BookTickerSample[] }> {
  ...
  const grouped = await Promise.all(expectedSymbols.map(async (symbol) => {
    ...
    for (let asOfMs = input.expectedCoverage.startMs + HOUR_MS - 1; asOfMs < input.expectedCoverage.endMs; asOfMs += HOUR_MS) {
...
/** Streams official monthly bookTicker ZIPs plus explicit immutable daily repairs into one exact BBO mark per completed-candle decision tick. */
