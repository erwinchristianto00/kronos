...
async function bookTickerSamples(input: { path: string; relativePath: string; symbol: string; startMs: number; endMs: number; sourceStartMs: number; sourceEndMs: number; sourceHash: string; maxQuoteAgeMs: number; initialState?: BookTickerState }): Promise<{ samples: BookTickerSample[]; carry: BookTickerState }> {
  ...
  const unzip = spawn("unzip", ["-p", input.path], { stdio: ["ignore", "pipe", "pipe"] }); const sampler = spawn("awk", ["-F,", "-v", `start=${input.startMs}`, "-v", `end=${input.endMs}`, "-v", `sourceStart=${input.sourceStartMs}`, "-v", `sourceEnd=${input.sourceEndMs}`, "-v", `interval=${HOUR_MS}`, "-v", `initialTime=${initial?.eventTimeMs ?? 0}`, "-v", `initialUpdate=${initial?.updateId ?? 0}`, "-v", `initialBidPrice=${initial?.bidPrice ?? 0}`, "-v", `initialBidQty=${initial?.bidQuantity ?? 0}`, "-v", `initialAskPrice=${initial?.askPrice ?? 0}`, "-v", `initialAskQty=${initial?.askQuantity ?? 0}`, awk], { stdio: ["pipe", "pipe", "pipe"] });
