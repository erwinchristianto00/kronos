...
const STALE_CACHE_ALERT_COUNT = 3;
...
export class FuturesReferenceHealthTracker {
  ...
  recordCacheEvent(event: FuturesReferenceCacheEvent): void {
    ...
    if (event.type === "STALE_CACHE_REJECTED") {
      ...
      this.fail(event.symbol, `stale cache rejected after ${Math.round(event.ageMs)}ms`, event.atMs);
      return;
    }
  ...
  /** Records one accepted pricing reference, whether reached by sizing or a read-only health probe. */
  recordReferenceUsed(reference: FuturesMarketReference): void {
    const entry = this.entry(reference.symbol);
  ...
  snapshot(watchSymbols: string[] = []): FuturesReferenceHealthSnapshot {
    ...
    if (this.staleCacheRejected >= STALE_CACHE_ALERT_COUNT) {
      alerts.push({
        code: "STALE_CACHE_INCREASING",
        severity: "WATCH",
        message: `${this.staleCacheRejected} stale cache entries were rejected, never reused.`,
      });
    }
