...
export type SymbolReliabilityPersistenceStatus = "HEALTHY" | "RECOVERED_FROM_BACKUP" | "UNAVAILABLE";

/**
 * Durable-store state, surfaced with every runtime snapshot.  UNAVAILABLE is deliberately
 * distinct from INSUFFICIENT_DATA: the latter preserves the baseline eligibility policy, while
 * the former means the circuit breaker's prior state cannot be proven durable and new V1 basket
 * formation must stand down.
 */
export interface SymbolReliabilityPersistence {
  status: SymbolReliabilityPersistenceStatus;
...
export interface SymbolReliabilityFormationDecision {
  ...
  evaluatedAt: string;
  evaluationId: string;
  persistence: SymbolReliabilityPersistence;
...
type ReliabilityStateRead = {
  exists: boolean;
  state: ReliabilityState | null;
  reason: string | null;
};
...
function stateFromUnknown(value: unknown): ReliabilityState | null {
  const parsed = asRecord(value);
  if (!parsed || parsed.version !== 1) return null;
  if (parsed.evaluationCycle !== undefined && (!Number.isInteger(parsed.evaluationCycle) || (parsed.evaluationCycle as number) < 0)) return null;
  if (parsed.lastEvaluationDay !== undefined && parsed.lastEvaluationDay !== null && typeof parsed.lastEvaluationDay !== "string") return null;
  if (parsed.lastEvidenceFingerprint !== undefined && parsed.lastEvidenceFingerprint !== null && typeof parsed.lastEvidenceFingerprint !== "string") return null;
  const statuses = parsed.statuses === undefined ? {} : asRecord(parsed.statuses);
  if (!statuses) return null;
  if (parsed.latest !== undefined && parsed.latest !== null && !asRecord(parsed.latest)) return null;
  if (parsed.formationDecisions !== undefined && !Array.isArray(parsed.formationDecisions)) return null;
  return {
    version: 1,
    evaluationCycle: Number.isInteger(parsed.evaluationCycle) ? parsed.evaluationCycle as number : 0,
    ...
    statuses: statuses as Record<string, StoredStatus>,
    latest: (parsed.latest as SymbolReliabilitySnapshot | null | undefined) ?? null,
    formationDecisions: (parsed.formationDecisions as SymbolReliabilityFormationDecision[] | undefined) ?? [],
  };
}
...
export class CrossSectionalSymbolReliabilityStore {
  ...
  private state: ReliabilityState = emptyState();
  private persistence: SymbolReliabilityPersistence = {
    status: "HEALTHY",
    source: "PRIMARY",
    reason: null,
    recoveredAt: null,
  ...
  /**
   * A write failure can retry from the in-memory, known-good state. A primary+backup load
   * failure cannot: starting an empty breaker would silently release a prior quarantine.
   */
  private persistenceRecoverable = true;
  ...
  constructor(dataDir = "data") {
    ...
    try {
      ...
      const primary = this.readState(this.file, "primary");
      const backup = this.readState(this.backupFile, "backup");
      if (primary.state) {
        this.state = primary.state;
        // Create (or repair) the recovery copy before this store is allowed to govern a new
        // basket. An unusable backup is archived first so no diagnostic evidence is destroyed.
        if (!backup.state) {
          if (backup.exists && !this.archiveInvalidLedger(this.backupFile)) {
            this.markUnavailable("backup ledger is invalid and could not be preserved before repair", false);
            return;
          }
          this.save();
        }
      ...
      if (backup.state) {
        ...
        if (primary.exists && !this.archiveInvalidLedger(this.file)) {
          this.markUnavailable("primary ledger is invalid and could not be preserved before recovery", false);
          return;
        }
        const recoveredAt = nowIso(Date.now());
        if (this.save()) {
          this.persistence = {
            status: "RECOVERED_FROM_BACKUP",
            source: "BACKUP",
            reason: primary.exists
              ? "primary ledger was invalid; restored atomically from the last valid backup"
              : "primary ledger was missing; restored atomically from the last valid backup",
            recoveredAt,
          };
        }
        return;
      ...
      if (primary.exists || backup.exists) {
        const primaryReason = primary.reason ?? "primary ledger unavailable";
        const backupReason = backup.reason ?? "backup ledger unavailable";
        this.markUnavailable(primaryReason + "; " + backupReason, false);
      }
    } catch (error) {
      this.markUnavailable("unable to initialize reliability persistence: " + errorMessage(error), false);
    }
  }

  private readState(file: string, label: string): ReliabilityStateRead {
    if (!existsSync(file)) return { exists: false, state: null, reason: null };
    try {
      const state = stateFromUnknown(JSON.parse(readFileSync(file, "utf-8")) as unknown);
      return state
        ? { exists: true, state, reason: null }
        : { exists: true, state: null, reason: label + " ledger has an unsupported or invalid shape" };
    } catch (error) {
      return { exists: true, state: null, reason: label + " ledger could not be parsed: " + errorMessage(error) };
    }
  }

  private archiveInvalidLedger(file: string): string | null {
    try {
      const archived = file + ".corrupt-" + Date.now();
  ...
  private markUnavailable(reason: string, recoverable: boolean): void {
    this.persistence = { status: "UNAVAILABLE", source: null, reason, recoveredAt: null };
    this.persistenceRecoverable = recoverable;
  }
  ...
  private atomicWrite(file: string, serialized: string): void {
    ...
    try {
      ...
      fileHandle = openSync(temporary, "r");
      ...
      if (fileHandle !== null) {
        try { closeSync(fileHandle); } catch { /* best-effort cleanup */ }
      }
  ...
  private save(): boolean {
    if (!this.persistenceRecoverable) return false;
    try {
      const serialized = JSON.stringify(this.state);
      this.atomicWrite(this.file, serialized);
      this.atomicWrite(this.backupFile, serialized);
      if (this.persistence.status === "UNAVAILABLE") {
        this.persistence = { status: "HEALTHY", source: "PRIMARY", reason: null, recoveredAt: null };
      }
      return true;
    } catch (error) {
      // The in-memory state remains intact, so a later evaluation can retry. Until a durable
      // primary+backup pair exists, Formation must not open a new reliability-governed basket.
      this.markUnavailable("reliability ledger write failed: " + errorMessage(error), true);
      return false;
    }
  }

  private withPersistence(snapshot: SymbolReliabilitySnapshot): SymbolReliabilitySnapshot {
    return {
      ...snapshot,
      persistence: { ...this.persistence },
      lastFormationDecision: this.state.formationDecisions.at(-1) ?? snapshot.lastFormationDecision,
    };
  }

  private unavailableSnapshot(nowMs: number): SymbolReliabilitySnapshot {
    if (this.state.latest) return this.withPersistence(this.state.latest);
    return {
      version: SYMBOL_RELIABILITY_VERSION,
      ...
      evaluationId: "sr-v1-persistence-unavailable",
  ...
  evaluate(input: { baskets: readonly ExecutorBasket[]; universe: readonly string[]; nowMs?: number }): SymbolReliabilitySnapshot {
    ...
    if (this.persistence.status === "UNAVAILABLE" && !this.persistenceRecoverable) {
      return this.unavailableSnapshot(nowMs);
    }
    ...
    if (!evidenceChanged && !scheduled && this.state.latest) {
      if (this.persistence.status === "UNAVAILABLE" && !this.save()) {
        return this.unavailableSnapshot(nowMs);
      }
      return this.withPersistence(this.state.latest);
    }
    ...
    if (!this.save()) return this.unavailableSnapshot(nowMs);
    return this.withPersistence(snapshot);
  }

  recordFormationDecision(decision: SymbolReliabilityFormationDecision): boolean {
    if (this.persistence.status === "UNAVAILABLE" && !this.persistenceRecoverable) return false;
  ...
  latest(): SymbolReliabilitySnapshot | null {
    return this.state.latest ? this.withPersistence(this.state.latest) : null;
  }
}
