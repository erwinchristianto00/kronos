...
/** Natural adjacent-candle reject rate + reasons in the REAL source under the SAME tolerances used for seams. */
export function naturalAdjacencyReject(frames: readonly CommonMarketFrame[], symbols: string[], tol: StitchTolerances = DEFAULT_STITCH_TOLERANCES): { rate: number; reasons: Record<string, number>; n: number } {
  ...
  return { rate: n ? rejected / n : 0, reasons, n };
}
 ...
 */
export function computeSeamRealism(generatedSeams: { rejected: boolean; reasons: string[]; group: string | number }[], natural: { rate: number }, rng: { nextInt(a: number, b: number): number }, iterations = 1000): SeamRealismResult {
  ...
  const excessItems = items.map((it) => ({ value: it.value - natural.rate, group: it.group }));
  const excessCi = groupedBootstrapMeanCI(excessItems, rng, iterations);
