...
const listing = buildAuthoritativeTimelineArtifact({ artifactKind: "LISTING_DELISTING_TIMELINE", source: "Binance original English futures launch evidence", sourceProvenance: announcementSource, archiveBundle: (() => { const { contents: _contents, ...identity } = announcementBundle; return identity; })(), generatedAtMs, generationSha: gitCommit, expectedCoverage: executionCoverage, rows: timelineRows.map((row) => ({ ...row, status: "LISTED" })) }); persist({ rows: listing.canonicalRows, manifest: listing.manifest });
const futures = buildAuthoritativeTimelineArtifact({ artifactKind: "FUTURES_AVAILABILITY_TIMELINE", source: "Binance original English futures launch evidence", sourceProvenance: announcementSource, archiveBundle: (() => { const { contents: _contents, ...identity } = announcementBundle; return identity; })(), generatedAtMs, generationSha: gitCommit, expectedCoverage: executionCoverage, rows: timelineRows.map((row) => ({ ...row, available: true })) }); persist({ rows: futures.canonicalRows, manifest: futures.manifest });
