...
describe("Tier-1 artifact assembly", () => {
  ...
  it("accepts only canonical Kronos episode evidence and uses strictly prior candles for PIT risk", () => {
    expect(() => buildCanonicalEpisodeArtifact({ source: "calendar-day", sourceProvenance: fixtureSourceProvenance("calendar", "0000000"), generatedAtMs: 0, generationSha: "x", expectedCoverage: expected, events: [] })).toThrow("FOUNDRY_CANONICAL_EPISODE_SOURCE_INVALID");
    const episodes = buildCanonicalEpisodeArtifact({ source: "canonical-kronos:market-cause-export", sourceProvenance: { ...fixtureSourceProvenance("episodes", "0000000"), provenanceType: "KRONOS_CANONICAL_LEDGER" }, generatedAtMs: 0, generationSha: "x", expectedCoverage: expected, events: [{ symbol: "BTCUSDT", decisionTimeMs: 0, marketEpisodeId: "cause-A", sourceHash: "event", canonicalAlgorithm: "PERSISTED_MARKET_CAUSE" }, { symbol: "BTCUSDT", decisionTimeMs: H - 1, marketEpisodeId: "cause-A", sourceHash: "event", canonicalAlgorithm: "PERSISTED_MARKET_CAUSE" }] });
  ...
  it("assembles only complete coherent artifacts and rejects conflicts or incomplete ranking", () => {
    ...
    const smokeSpec: TournamentExperimentSpec = {
      tournamentVersion: "kronos-research-tournament-v1", gitCommit: "fixture", strategyVersion: "fixture", randomSeed: 1, capabilityTier: "TIER_1_BASELINE",
