...
describe("real Tier-1 provenance gate", () => {
  ...
  it("permanently isolates fixture artifacts, registries, and rankings from REAL_TIER1", () => {
    ...
    const synthetic = buildFoundryArtifact({ artifactKind: "COMPLETED_CANDLES", schemaVersion: FOUNDRY_SCHEMA_V1, source: "synthetic", sourceProvenance: { ...realProvenance("a".repeat(64)), provenanceType: "SYNTHETIC" }, units: { price: "USDT" }, generatedAtMs: 1, generationSha: "abcdef0", expectedCoverage: expected, rows: [{ symbol: "BTCUSDT", openTimeMs: 0, closeTimeMs: H - 1, open: 100, high: 101, low: 99, close: 100, volume: 1, sourceHash: "row" }] });
    expect(() => assertRealTier1ArtifactProvenance([{ manifest: synthetic.manifest, rows: synthetic.canonicalRows }], {})).toThrow("FOUNDRY_REAL_TIER1_FIXTURE_OR_PLACEHOLDER");
    ...
    const relabeledFixture = { ...manifest, empiricalClassification: "REAL_SOURCE_BACKED" as const, spec: { ...manifest.spec, researchMode: "REAL_TIER1" as const } };
