...
describe("Foundry semantic strictness", () => {
  ...
  it("streams checksum-verified Binance Vision bookTicker ZIPs into exact canonical hourly PIT marks", async () => {
    const root = mkdtempSync(join(tmpdir(), "foundry-binance-vision-bookticker-")); const startMs = Date.UTC(2023, 4, 1); const endMs = startMs + 2 * H;
    ...
    try {
      ...
      const imported = await importBinanceVisionUsdMRawBookTickerLiquidityArchive({ root, expectedCoverage: coverage, candleRows: candles.rows as never[], maxQuoteAgeMs: H, source: "Binance Vision BBO plus completed volume", sourceProvenance: provenance(bboBundle.archiveBundleHash), generatedAtMs: 1, generationSha: "abcdef0" });
      expect(imported.rows).toEqual(expect.arrayContaining([expect.objectContaining({ asOfMs: startMs + H - 1, validUntilMs: startMs + H - 1, volume: 7, liquidityNotional: 200 }), expect.objectContaining({ asOfMs: startMs + 2 * H - 1, volume: 8, liquidityNotional: 404 })]));
      expect(imported.manifest.archiveBundle?.archiveBundleHash).toBe(bboBundle.archiveBundleHash);
      await expect(importBinanceVisionUsdMRawBookTickerLiquidityArchive({ root, expectedCoverage: coverage, candleRows: candles.rows as never[], maxQuoteAgeMs: 0, source: "Binance Vision BBO plus completed volume", sourceProvenance: provenance(bboBundle.archiveBundleHash), generatedAtMs: 1, generationSha: "abcdef0" })).rejects.toThrow("FOUNDRY_BINANCE_VISION_BOOKTICKER_PIT_INVALID");
    } finally { rmSync(root, { recursive: true, force: true }); }
  });
});
