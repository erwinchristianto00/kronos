...
describe("Symbol Reliability V1", () => {
  ...
  it("recovers a corrupt primary only from a valid atomic backup, and never silently starts empty when both copies are invalid", () => {
    const dir = mkdtempSync(join(tmpdir(), "symbol-reliability-recovery-"));
    const firstStore = new CrossSectionalSymbolReliabilityStore(dir);
    const first = firstStore.evaluate({ baskets: deterioratingEpisodes(8), universe: UNIVERSE, nowMs: NOW });
    const primary = join(dir, "cross-sectional-symbol-reliability-v1.json");
    const backup = primary + ".bak";
    expect(existsSync(primary)).toBe(true);
    expect(existsSync(backup)).toBe(true);

    writeFileSync(primary, "{not-json", "utf-8");
    const recovered = new CrossSectionalSymbolReliabilityStore(dir);
    expect(recovered.latest()?.persistence).toMatchObject({
      status: "RECOVERED_FROM_BACKUP",
      source: "BACKUP",
    });
    expect(recovered.latest()?.evaluationCycle).toBe(first.evaluationCycle);

    writeFileSync(primary, "{not-json", "utf-8");
    writeFileSync(backup, "{also-not-json", "utf-8");
    const unavailable = new CrossSectionalSymbolReliabilityStore(dir);
    const snapshot = unavailable.evaluate({ baskets: deterioratingEpisodes(9), universe: UNIVERSE, nowMs: NOW + DAY });
    expect(snapshot.persistence).toMatchObject({ status: "UNAVAILABLE" });
