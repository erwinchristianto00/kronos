...
const horizonTimeTaipei = new Intl.DateTimeFormat('en-GB', {
  timeZone: 'Asia/Taipei', year: 'numeric', month: 'short', day: '2-digit',
  hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: false,
});
const horizonTimeUtc = new Intl.DateTimeFormat('en-GB', {
...
function formatHorizonClose(closeAtMs: number | null): string {
  if (closeAtMs == null) return 'waktu HORIZON tidak tersedia';
  const closeAt = new Date(closeAtMs);
  if (Number.isNaN(closeAt.getTime())) return 'waktu HORIZON tidak valid';
  return `${horizonTimeTaipei.format(closeAt)} Taipei / ${horizonTimeUtc.format(closeAt)} UTC`;
}
          ...
          {xsecInstances.some(({ status: xs }) => (xs?.openBaskets ?? []).length > 0) && (
            <div style={{ margin: '6px 0', fontSize: 12 }}>
              {xsecInstances.flatMap(({ label, status: xs }) => (xs?.openBaskets ?? []).map((b) => {
                ...
                // `closesAtMs` is the research measurement horizon (48h), not necessarily the execution
                // HORIZON cap. Resolve the policy frozen for THIS basket first so a post-entry policy
                // deployment never silently moves its displayed close time.
                ...
                return (
                  <div key={b.basketId} style={{ display: 'flex', gap: 14, padding: '2px 0', flexWrap: 'wrap' }}>
                    ...
                    <span className="tone-measure" title={`HORIZON source: ${horizon.source}${horizon.earlyExitPossible ? '. TP/SL/emergency may close earlier.' : ''}`}>
                      close HORIZON {horizon.capHours == null ? '—' : `${horizon.capHours}h`}: {formatHorizonClose(horizon.closeAtMs)}{hoursLeft == null ? '' : ` · ${hoursLeft.toFixed(1)}h lagi`}
                    </span>
