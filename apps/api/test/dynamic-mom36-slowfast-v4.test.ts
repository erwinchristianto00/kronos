...
function evaluationInput(
  ...
  return {
    ...
    admissionExternalReason: admissionPassed ? null : "TEST_EXTERNAL_GUARD",
    legacyAdmissionScoreGap: 0.1,
    legacyAdmissionPassed: admissionPassed,
    strategyVersion,
...
describe("Dynamic MOM36 v4 — recovered legacy SLOW_AND_FAST", () => {
  ...
  it("V6 converts an infeasible 4L2S only to the nearest fully strict-valid 6L0S allocation", () => {
    const rows = [
      row("P01", 0.20, 0.020), row("P02", 0.19, 0.019), row("P03", 0.18, 0.018),
      row("P04", 0.17, 0.017), row("P05", 0.16, 0.016), row("P06", 0.15, 0.015),
      // Both slow-bearish names are bouncing on FAST4H, so V4 must not short either one.
      row("N01", -0.20, 0.010), row("N02", -0.19, 0.009),
    ];
    const v4 = buildDynamicMom36Formation({
      activeUniverse: rows,
      maxPerCluster: 0,
      continuation: continuation("NO_EDGE"),
      continuationOnly: true,
    ...
    const v6 = buildDynamicMom36Formation({
      activeUniverse: rows,
      maxPerCluster: 0,
      continuation: continuation("NO_EDGE"),
      continuationOnly: true,
      slowFastMode: "STRICT_DIRECTIONAL_FEASIBILITY",
    });
    const evaluated = evaluateDynamicMom36Formation(
      evaluationInput(rows, true, DYNAMIC_MOM36_CONTINUATION_SLOWFAST_FEASIBILITY_SL2_MFE30_36H_V6),
    );

    // Mutation guard: the incumbent V4 policy remains a hard no-entry on this exact snapshot.
    expect(v4.finalAllocation).toMatchObject({ label: "4L2S" });
    expect(v4.selectionSource).toBe("STRICT_SLOW_FAST");
    expect(v4.selection.insufficientReason).toBe("INSUFFICIENT_SLOW_FAST_ALIGNED_LEGS");

    expect(v6.requestedAllocation).toMatchObject({ label: "4L2S" });
    expect(v6.finalAllocation).toMatchObject({ label: "6L0S" });
    expect(v6.selectionSource).toBe("STRICT_SLOW_FAST_DIRECTIONAL_FEASIBILITY");
    expect(v6.selection.selectedLongs.map((candidate) => candidate.symbol)).toEqual(["P01", "P02", "P03", "P04", "P05", "P06"]);
    expect(v6.selection.selectedShorts).toHaveLength(0);
    expect(v6.directionalFeasibility).toMatchObject({
      active: true,
      directionalPrior: "LONG",
      outcome: "FALLBACK_APPLIED",
      effectiveAllocation: { label: "6L0S" },
    });
    expect(v6.directionalFeasibility.attempts.map((attempt) => [attempt.allocation.label, attempt.complete])).toEqual([
      ["5L1S", false], ["6L0S", true],
    ]);

    expect(evaluated.basket).toMatchObject({ longK: 6, shortK: 0, longCapitalWeight: 1, shortCapitalWeight: 0 });
    expect(evaluated.snapshot).toMatchObject({
  ...
  it("V6 still abstains when fewer than six strict-valid legs exist on the dominant side", () => {
    const rows = [
      row("P01", 0.20, 0.020), row("P02", 0.19, 0.019), row("P03", 0.18, 0.018),
      row("P04", 0.17, 0.017), row("P05", 0.16, 0.016), row("P06", 0.15, -0.015),
      row("N01", -0.20, 0.010), row("N02", -0.19, 0.009),
    ...
    expect(evaluated.basket).toBeNull();
    expect(evaluated.formation).toMatchObject({
      requestedAllocation: { label: "4L2S" },
      ...
      directionalFeasibility: {
        ...
        attempts: [
          ...
          { allocation: { label: "6L0S" }, complete: false },
        ],
      },
    });
    expect(evaluated.noEntryReason).toBe("INSUFFICIENT_SLOW_FAST_ALIGNED_LEGS");
  });

  it("V6 mirrors the feasibility resolver to 0L6S without borrowing an unaligned long", () => {
    const rows = [
      row("P01", 0.20, -0.010), row("P02", 0.19, -0.009),
      row("N01", -0.20, -0.020), row("N02", -0.19, -0.019), row("N03", -0.18, -0.018),
      row("N04", -0.17, -0.017), row("N05", -0.16, -0.016), row("N06", -0.15, -0.015),
    ];
    const evaluated = evaluateDynamicMom36Formation(
      evaluationInput(rows, true, DYNAMIC_MOM36_CONTINUATION_SLOWFAST_FEASIBILITY_SL2_MFE30_36H_V6),
    );

    expect(evaluated.basket).toMatchObject({ longK: 0, shortK: 6, longCapitalWeight: 0, shortCapitalWeight: 1 });
    expect(evaluated.snapshot).toMatchObject({
      requestedAllocation: { label: "2L4S" },
      finalAllocation: { label: "0L6S" },
      selectionSource: "STRICT_SLOW_FAST_DIRECTIONAL_FEASIBILITY",
      directionalFeasibility: {
        directionalPrior: "SHORT",
        outcome: "FALLBACK_APPLIED",
        attempts: [
          { allocation: { label: "1L5S" }, complete: false },
          { allocation: { label: "0L6S" }, complete: true },
        ],
      },
    });
  });

  it("V6 retains a fully feasible requested allocation and does not search directionally", () => {
    const rows = [
      row("P01", 0.20, 0.020), row("P02", 0.19, 0.019), row("P03", 0.18, 0.018), row("P04", 0.17, 0.017),
      row("N01", -0.20, -0.020), row("N02", -0.19, -0.019),
    ];
    const formation = buildDynamicMom36Formation({
      activeUniverse: rows,
      maxPerCluster: 0,
    ...
    expect(formation.requestedAllocation).toMatchObject({ label: "4L2S" });
    expect(formation.finalAllocation).toMatchObject({ label: "4L2S" });
    expect(formation.selectionSource).toBe("STRICT_SLOW_FAST");
    expect(formation.directionalFeasibility).toMatchObject({
      active: true,
      outcome: "REQUESTED_STRICT_FEASIBLE",
      attempts: [],
      effectiveAllocation: { label: "4L2S" },
    });
  });
