...
function structuralHourlyHistory(decisionAt: number, resistance = 104): FuturesKline[] {
  const hour = 60 * 60_000;
  const firstOpen = Math.floor((decisionAt - 1) / hour) * hour - 7 * hour;
  const highs = [101, 102, resistance, 103, 102, 103, 102];
  return highs.map((high, index) => {
    const openTime = firstOpen + index * hour;
    return {
      openTime,
...
function attachStructuralEconomics(signal: DailyRangeSignal): DailyRangeSignal {
  const planning = prepareDailyRangeEconomics({
    side: signal.direction,
    route: signal.entryPolicy ?? "CONTINUATION",
    symbol: signal.symbol,
    batchTimestampMs: signal.signalTimestampMs - 1,
    rawStructuralStop: structuralStopForDailyRangeSignal(signal),
    stopSource: signal.entryPolicy === "FADE" ? "FAILED_BREAKOUT_EXTREME" : "FLIPPED_RANGE_RETEST",
    structuralTarget: signal.direction === "LONG"
      ? {
        target: 104,
        targetSource: "CONFIRMED_1H_SWING_HIGH",
        targetLevelType: "SWING_HIGH",
        targetSourceTimeframe: "1h",
        confirmedAt: new Date(signal.signalTimestampMs - 60 * 60_000).toISOString(),
        sourceOpenTime: signal.signalTimestampMs - 3 * 60 * 60_000,
      }
      : {
        target: 86,
        targetSource: "CONFIRMED_1H_SWING_LOW",
        targetLevelType: "SWING_LOW",
        targetSourceTimeframe: "1h",
        confirmedAt: new Date(signal.signalTimestampMs - 60 * 60_000).toISOString(),
        sourceOpenTime: signal.signalTimestampMs - 3 * 60 * 60_000,
      },
    bbo: { bid: 100, ask: 100, observedAt: new Date(signal.signalTimestampMs).toISOString(), receivedAt: new Date(signal.signalTimestampMs).toISOString(), sourceTime: signal.signalTimestampMs },
    filter: filter(signal.symbol),
    frictionModel: conservativeFallbackFrictionModel(new Date(signal.signalTimestampMs).toISOString()),
    bboMaxAgeMs: 30_000,
    allocationAtMs: signal.signalTimestampMs + 1_000,
    atr4hFeature: null,
  });
  if (!planning.ok) throw new Error(`fixture economics failed: ${planning.reason}`);
  signal.economics = planning.economics;
  signal.geometry = planning.economics.geometry;
  return signal;
}
...
describe("daily-4h-range-acceptance-2r-v1", () => {
  ...
  it("records actual fills, fees, entry/exit slippage, and cancels both owned siblings on a controlled lane close", async () => {
    ...
    // A chart archive has no authority over a real exchange close. This fixture
    // intentionally has no 4H history, so the image is unavailable while the
    // confirmed fill, accounting, and sibling cancellation remain terminal.
    expect(trade.closedChartSnapshot?.status).toBe("UNAVAILABLE");
  ...
  it("freezes a pre-Structural-S/R open position even if it breaches the retired geometry bands", async () => {
    const now = { value: AT_0410 };
    const client = new FakeDailyClient();
    const { lane, store } = makeLane(client, now);
    store.arm(new Date(now.value).toISOString());
    ...
    expect(trade.status).toBe("OPEN");
    expect(client.placed.filter((order) => order.reduceOnly)).toHaveLength(0);
    expect((await client.getPositions("AAAUSDT")).some((position) => Math.abs(position.positionAmt) > 0)).toBe(true);
    expect((await client.getOpenAlgoOrders("AAAUSDT"))).toHaveLength(2);
  });

  it("marks a pre-Structural-S/R open position as frozen without recomputing its ATR", async () => {
    const now = { value: AT_0410 };
    const client = new FakeDailyClient();
    const { lane, store } = makeLane(client, now);
    store.arm(new Date(now.value).toISOString());
  ...
  it("does not apply the retired OPUSDT migration close to a frozen existing position", async () => {
    const now = { value: AT_0410 };
    const client = new FakeDailyClient();
    ...
    expect(trade.status).toBe("OPEN");
    expect((await client.getPositions("OPUSDT")).some((position) => Math.abs(position.positionAmt) > 0)).toBe(true);
    expect((await client.getOpenAlgoOrders("OPUSDT"))).toHaveLength(2);
  });

  it("keeps frozen legacy positions untouched even when foreign ownership is present", async () => {
    const now = { value: AT_0410 };
    const client = new FakeDailyClient();
    client.klines.set("OPUSDT|4h", atrFourHourHistory(now.value));
    const { lane, store } = makeLane(
    ...
    expect(trade.geometryMigration).toMatchObject({ status: "FROZEN", reason: "FROZEN_PRE_STRUCTURAL_POLICY", action: "KEPT" });
    expect(trade.status).toBe("OPEN");
    expect(client.placed.filter((order) => order.reduceOnly)).toHaveLength(0);
    expect((await client.getOpenAlgoOrders("OPUSDT"))).toHaveLength(2);
  });
  ...
  it("accepts the Testnet first-breakout continuation as one completed C1, without inventing a C2", () => {
    const window = newYorkDailyRangeWindow(Date.UTC(2026, 7, 26, 12));
    ...
    const symbolState: DailyRangeSymbolState = {
      lastProcessedBarOpenTime: null,
      previousClosedCandle: null,
      longCount: 0,
      shortCount: 0,
    ...
    const day: DailyRangeDayState = {
      ...
      levels: { AAAUSDT: routeLevel },
      invalidReferenceSymbols: [],
      symbolStates: { AAAUSDT: symbolState },
      strategyVersion: "daily-4h-range-auto-route-ny-meme-v3",
      strategyMode: "AUTO_ROUTE_NY_V3",
      autoRouteEntryMode: "CONTINUATION_FIRST_OUTSIDE_CLOSE",
      referenceTimezone: "America/New_York",
      referenceRangeOpenTime: window.rangeOpenTime,
      referenceRangeCloseTime: window.rangeCloseTime,
      entryWindowCloseTime: window.entryWindowCloseTime,
    };
    const signal = (lane as unknown as {
      applyCandle(dayArg: DailyRangeDayState, levelArg: DailyRangeLevel, stateArg: DailyRangeSymbolState, rowArg: DailyRangeCandle): DailyRangeSignal | null;
    }).applyCandle(day, routeLevel, symbolState, candle(window.rangeCloseTime, 101, 100.5, 102));
    ...
    expect(signal).toMatchObject({
      ...
      reason: null,
      confirmationBar1: { openTime: window.rangeCloseTime },
      confirmationBar2: { openTime: window.rangeCloseTime },
    });
    const pending = (lane as unknown as {
      createPendingTrade(signalArg: DailyRangeSignal, options?: { persist?: boolean }): { entryTiming?: string | null; structuralStopRaw: number } | null;
    }).createPendingTrade(signal!, { persist: false });
    expect(pending).toMatchObject({ entryTiming: "FIRST_OUTSIDE_CLOSE", structuralStopRaw: 100 });
    expect(store.getState().signals).toHaveLength(1);
  });

  it("executes a structurally planned Testnet continuation, but keeps its otherwise identical Live candidate shadow-only without a reservation, slot, intent, or order", async () => {
    const window = newYorkDailyRangeWindow(Date.UTC(2026, 7, 26, 12));
    const now = { value: window.rangeCloseTime + 16 * 60_000 };
    const policyFor = (route: "CONTINUATION" | "FADE", breakoutDirection: "UP" | "DOWN") => {
      const policy = dailyRangeRouteExitPolicyForSignal({
        route,
        originalBreakoutDirection: breakoutDirection,
        rangeHigh: 100,
        rangeLow: 90,
        effectiveAt: new Date(window.rangeCloseTime + 15 * 60_000).toISOString(),
        structuralSrEnabled: true,
      });
      if (!policy) throw new Error("missing Structural S/R route policy");
      return policy;
    };
    const testnetClient = new FakeDailyClient();
    testnetClient.now = now.value;
    const testnetStore = new DailyRangeLaneStore(dataDir(), "state.json", now.value);
    const testnetLane = new DailyRangeAcceptanceLane({
    ...
    testnetStore.arm(new Date(window.rangeCloseTime + 14 * 60_000).toISOString());
    const testnetContinuation = attachStructuralEconomics(autoRouteSignal({
    ...
    testnetContinuation.strategyVersion = "daily-4h-range-auto-route-ny-meme-v3";
    testnetContinuation.routeExitPolicy = policyFor("CONTINUATION", "UP");
    testnetStore.getState().signals.push(testnetContinuation);
    await (testnetLane as unknown as { executeFreshSignal(signalArg: DailyRangeSignal): Promise<void> }).executeFreshSignal(testnetContinuation);
    expect(testnetStore.getState().trades).toHaveLength(1);
    expect(testnetClient.placed.filter((order) => !order.reduceOnly)).toHaveLength(1);
    expect(testnetContinuation.reason).toBeNull();

    const liveClient = new FakeDailyClient();
    liveClient.now = now.value;
    const liveStore = new DailyRangeLaneStore(dataDir(), "state.json", now.value);
    const liveLane = new DailyRangeAcceptanceLane({
      client: liveClient,
      store: liveStore,
      getUniverse: () => ({ symbols: ["BBBUSDT"], source: "TEST" }),
      getShortBlocklist: () => new Set<string>(),
      entryClaims: { tryClaimEntrySymbol: () => true, releaseEntrySymbol: () => {} },
      environment: "mainnet",
      mainnetControls: mainnetControls({ continuationExecutionEnabled: false }),
    ...
    liveStore.arm(new Date(window.rangeCloseTime + 14 * 60_000).toISOString());
    const liveContinuation = attachStructuralEconomics(autoRouteSignal({
      symbol: "BBBUSDT", decisionAt: window.rangeCloseTime + 15 * 60_000, route: "CONTINUATION", direction: "LONG", breakoutDirection: "UP", breakoutExtreme: 103,
    }));
    liveContinuation.strategyVersion = "daily-4h-range-auto-route-ny-meme-v3";
    liveContinuation.routeExitPolicy = policyFor("CONTINUATION", "UP");
    liveStore.getState().signals.push(liveContinuation);
    await (liveLane as unknown as { executeFreshSignal(signalArg: DailyRangeSignal): Promise<void> }).executeFreshSignal(liveContinuation);
    expect(liveContinuation).toMatchObject({
      reason: "LIVE_CONTINUATION_EXECUTION_DISABLED",
      entryAttemptedAt: null,
      routeExecutionEnabled: false,
      executionEligible: false,
      tradeId: null,
    });
    expect(liveStore.getState().trades).toHaveLength(0);
    expect(liveClient.placed).toHaveLength(0);
    expect(liveClient.algoPlaced).toHaveLength(0);

    const liveFade = attachStructuralEconomics(autoRouteSignal({
      symbol: "BBBUSDT", decisionAt: window.rangeCloseTime + 20 * 60_000, route: "FADE", direction: "SHORT", breakoutDirection: "UP", breakoutExtreme: 104,
    }));
    liveFade.strategyVersion = "daily-4h-range-auto-route-ny-meme-v3";
    liveFade.routeExitPolicy = policyFor("FADE", "UP");
    liveStore.getState().signals.push(liveFade);
    await (liveLane as unknown as { executeFreshSignal(signalArg: DailyRangeSignal): Promise<void> }).executeFreshSignal(liveFade);
    expect(liveStore.getState().trades).toHaveLength(1);
    expect(liveClient.placed.filter((order) => !order.reduceOnly)).toHaveLength(1);
  });

  it("fully evaluates a Live continuation before shadowing it, so an executable Fade still receives the only available slot", async () => {
    const window = newYorkDailyRangeWindow(Date.UTC(2026, 7, 26, 12));
    const decisionAt = window.rangeCloseTime + 15 * 60_000;
    const now = { value: decisionAt + 1_000 };
    const client = new FakeDailyClient();
    client.now = now.value;
    client.klines.set("AAAUSDT|1h", structuralHourlyHistory(decisionAt));
    const store = new DailyRangeLaneStore(dataDir(), "state.json", now.value);
    const friction = conservativeFallbackFrictionModel(new Date(decisionAt - 1_000).toISOString(), new Date(decisionAt - 1_000).toISOString(), "mainnet");
    store.getState().frictionModels.push(friction);
    store.getState().frictionModelByUtcDate[new Date(decisionAt).toISOString().slice(0, 10)] = friction.id;
    const lane = new DailyRangeAcceptanceLane({
      client,
      store,
      getUniverse: () => ({ symbols: ["AAAUSDT", "BBBUSDT"], source: "TEST", poolEvidence: poolEvidence(["AAAUSDT", "BBBUSDT"]) }),
      getShortBlocklist: () => new Set<string>(),
      entryClaims: { tryClaimEntrySymbol: () => true, releaseEntrySymbol: () => {} },
      environment: "mainnet",
      mainnetControls: mainnetControls({ continuationExecutionEnabled: false, maxOpenTrades: 1 }),
      allocatorMode: "ECONOMIC_QUALITY_BASELINE",
      strategyMode: "AUTO_ROUTE_NY_V3",
      autoRouteEntryMode: "FOLLOW_THROUGH_OR_FIRST_REENTRY",
      structuralSrPolicyEnabled: true,
    ...
    const policy = (route: "CONTINUATION" | "FADE") => {
      const snapshot = dailyRangeRouteExitPolicyForSignal({
        route,
        originalBreakoutDirection: "UP",
        rangeHigh: 100,
        rangeLow: 90,
        effectiveAt: new Date(decisionAt).toISOString(),
        structuralSrEnabled: true,
      });
      if (!snapshot) throw new Error("missing structural policy");
      return snapshot;
    };
    const continuation = autoRouteSignal({
      symbol: "AAAUSDT", decisionAt, route: "CONTINUATION", direction: "LONG", breakoutDirection: "UP", breakoutExtreme: 103,
    });
    continuation.strategyVersion = "daily-4h-range-auto-route-ny-meme-v3";
    continuation.routeExitPolicy = policy("CONTINUATION");
    continuation.research = {
      marketQuality: {
        capturedAt: new Date(decisionAt).toISOString(),
        poolCapturedAt: new Date(decisionAt - 1_000).toISOString(),
        pitQuality: "FULL_PIT",
        capturePhase: "FORWARD_BEFORE_ALLOCATION",
        bookSnapshotQuality: "AT_DECISION_BEFORE_ALLOCATION",
        bookObservedAt: new Date(decisionAt).toISOString(),
        bookReceivedAt: new Date(decisionAt).toISOString(),
        bookSourceTime: decisionAt,
        bestBid: 100,
        bestAsk: 100,
        quoteVolume24hUsd: 50_000_000,
        medianSpreadBps: 1,
        ...
        c5Pass: true,
        c6Pass: true,
        poolAudit: poolEvidence(["AAAUSDT"]).auditBySymbol.AAAUSDT ?? null,
      },
      features: null,
      counterfactual: null,
    };
    const fade = autoRouteSignal({
      symbol: "BBBUSDT", decisionAt, route: "FADE", direction: "SHORT", breakoutDirection: "UP", breakoutExtreme: 104,
    });
    fade.strategyVersion = "daily-4h-range-auto-route-ny-meme-v3";
    fade.routeExitPolicy = policy("FADE");
    fade.research = {
      marketQuality: {
        ...continuation.research.marketQuality!,
        poolAudit: poolEvidence(["BBBUSDT"]).auditBySymbol.BBBUSDT ?? null,
      },
      features: null,
      counterfactual: null,
    };
    store.getState().signals.push(continuation, fade);
    const day: DailyRangeDayState = {
      dateUtc: window.date,
      initializedAt: new Date(now.value).toISOString(),
      universeSymbols: ["AAAUSDT", "BBBUSDT"],
      universeSource: "TEST",
      levels: {
        AAAUSDT: { ...level("AAAUSDT"), dateUtc: window.date, fourHourOpenTime: window.rangeOpenTime, fourHourCloseTime: window.rangeCloseTime },
        BBBUSDT: { ...level("BBBUSDT"), dateUtc: window.date, fourHourOpenTime: window.rangeOpenTime, fourHourCloseTime: window.rangeCloseTime },
      },
      invalidReferenceSymbols: [],
      symbolStates: {},
      strategyVersion: "daily-4h-range-auto-route-ny-meme-v3",
    ...
    (lane as unknown as { captureSignalBatches(dayArg: DailyRangeDayState, actions: DailyRangeSignal[]): void }).captureSignalBatches(day, [continuation, fade]);
    const cohort = store.getState().signalCohorts[0]!;
    await (lane as unknown as { finalizeSignalBatch(batch: typeof cohort): Promise<void> }).finalizeSignalBatch(cohort);

    expect(continuation).toMatchObject({
      economics: { structurePolicyId: "daily-structural-sr-v1", targetSource: "CONFIRMED_1H_SWING_HIGH", structuralTarget: 104 },
      routeExecutionEnabled: false,
      executionEligible: false,
      reason: "LIVE_CONTINUATION_EXECUTION_DISABLED",
      entryAttemptedAt: null,
    ...
    expect(fade).toMatchObject({ routeExecutionEnabled: true, executionEligible: true, actuallySelected: true, actuallyExecuted: true });
    expect(cohort.allocation).toMatchObject({ availableSlots: 1, pendingReservationsAtBatch: 0, selectedSignalIds: [fade.signalId] });
    expect(store.getState().trades).toHaveLength(1);
    expect(store.getState().trades[0]).toMatchObject({ signalId: fade.signalId, entryPolicy: "FADE" });
    expect(client.placed.filter((order) => !order.reduceOnly)).toHaveLength(1);
  });

  it("keeps an active current-day V2 trade immutable while the V3 policy waits for it to close", async () => {
    const window = newYorkDailyRangeWindow(Date.UTC(2026, 7, 26, 12));
    const now = { value: window.rangeCloseTime + 10 * 60_000 };
    const client = new FakeDailyClient();
    const { lane, store } = makeLane(
    ...
    store.getState().days[`NY:${window.date}`] = {
      dateUtc: window.date,
      initializedAt: new Date(now.value).toISOString(),
      universeSymbols: ["AAAUSDT"],
      universeSource: "OLD_V2",
      levels: { AAAUSDT: routeLevel },
      invalidReferenceSymbols: [],
      symbolStates: {
        AAAUSDT: {
    ...
    store.getState().trades.push({
      dateUtc: window.date,
      status: "OPEN",
    } as unknown as DailyRangeTrade);
    store.arm(new Date(now.value).toISOString());

    await expect((lane as unknown as { ensureTodayRange(): Promise<boolean> }).ensureTodayRange()).resolves.toBe(false);

    expect(store.getState().days[`NY:${window.date}`]).toMatchObject({
      strategyVersion: "daily-4h-range-auto-route-ny-2r-v2",
      strategyMode: "AUTO_ROUTE_NY_V2",
      universeSource: "OLD_V2",
    });
    expect(store.getState().runtime.lastError).toContain("policy cutover pending: active Daily Range trade still belongs to daily-4h-range-auto-route-ny-2r-v2");
    expect(lane.getStatus()).toMatchObject({
      newEntriesEnabled: false,
      newEntryReason: "POLICY_CUTOVER_PENDING_ACTIVE_TRADE",
    });
  });
  ...
  it("builds the NY range from completed 5m candles and executes a fresh fade with its own bracket lineage", async () => {
    ...
    expect(trade.economics!.costRatio).toBeGreaterThan(0);
    expect(trade.actualCostRatio).toBeGreaterThan(0);
    ...
    expect(client.algoPlaced.map((row) => [row.type, row.side, row.triggerPrice]))
      .toEqual(expect.arrayContaining([["STOP_MARKET", "BUY", 101.5], ["TAKE_PROFIT_MARKET", "BUY", 90]]));
  ...
  it("keeps a wide structural stop as a diagnostic and lets it reach economic allocation", async () => {
    const window = newYorkDailyRangeWindow(Date.UTC(2026, 7, 26, 12));
    const now = { value: window.rangeCloseTime + 60_000 };
    const client = new FakeDailyClient();
    client.now = now.value;
    ...
    for (const symbol of universe) {
      ...
      const inside = candle(window.rangeCloseTime, 99, 95, 100);
      // Both rows are breakout fades. AAA keeps the same 1.5% stop geometry
      // as the route fixture above; BBB's frozen breakout extreme makes its
      // structural stop 4% away from the executable BBO.  That is beyond the
      // retired 3% band but still executable under the friction-inclusive
      // $0.25 risk budget.
      const firstOutside = symbol === "AAAUSDT"
        ? candle(window.rangeCloseTime + 5 * 60_000, 101, 100.5, 101.5)
        : candle(window.rangeCloseTime + 5 * 60_000, 103, 100.5, 104);
    ...
    expect(wide.geometry).toMatchObject({
      geometryPass: true,
      geometryRejectReason: null,
      admissionAuthority: "DIAGNOSTIC_ONLY",
      legacyDiagnosticReason: "STRUCTURAL_STOP_TOO_WIDE",
    });
    expect(wide.skipReason).toBeNull();
    expect(wide.actuallySelected).toBe(true);
    expect(wide.selectorRank).not.toBeNull();
    expect(cohort.allocation?.selectedSignalIds).toEqual(expect.arrayContaining([valid.signalId, wide.signalId]));
    expect(client.placed.filter((order) => !order.reduceOnly).map((order) => order.symbol).sort()).toEqual(["AAAUSDT", "BBBUSDT"]);
  });
  ...
  it("freezes native 1R continuation and 2R fade brackets without changing structural stops", async () => {
    ...
    expect(fadeTrade).toMatchObject({
      entryPolicy: "FADE",
      routeExitPolicy: { exitPolicyId: "daily-route-exit-v3-fade-bracket-only", tpMultipleR: 2, thesisInvalidationType: "NONE" },
      stopPrice: 102,
      takeProfitPrice: 96,
      rrTarget: 2,
    });
  ...
  it("keeps a fade bracket-only when price re-accepts the original breakout", async () => {
    const window = newYorkDailyRangeWindow(Date.UTC(2026, 7, 26, 12));
    const now = { value: window.rangeCloseTime + 16 * 60_000 };
    const client = new FakeDailyClient();
    client.now = now.value;
  ...
  it("admits a newly borrowable MOM36 symbol from the next completed candle without replaying an older breakout", async () => {
    const window = newYorkDailyRangeWindow(Date.UTC(2026, 7, 26, 12));
    const now = { value: window.rangeCloseTime + 10 * 60_000 };
    const client = new FakeDailyClient();
    client.now = now.value;
    const reference = Array.from({ length: 48 }, (_, index) => candle(
      window.rangeOpenTime + index * 5 * 60_000,
      95,
      90,
      100,
    ));
    // These two post-range outside closes would be a historical breakout if
    // the late admission were allowed to replay them.
    const oldOutside = [
      candle(window.rangeCloseTime, 101, 99, 102),
      candle(window.rangeCloseTime + 5 * 60_000, 102, 100, 103),
    ];
    client.klines.set("BASEUSDT|5m", reference);
    client.klines.set("BORROWUSDT|5m", [...reference, ...oldOutside]);
    let borrowerAvailable = false;
    const store = new DailyRangeLaneStore(dataDir(), "state.json", now.value);
    const lane = new DailyRangeAcceptanceLane({
      client,
      store,
      getUniverse: () => {
        const symbols = borrowerAvailable ? ["BASEUSDT", "BORROWUSDT"] : ["BASEUSDT"];
        return {
          symbols,
          borrowedSymbols: borrowerAvailable ? ["BORROWUSDT"] : [],
          source: "TEST",
          poolEvidence: poolEvidence(symbols),
        };
      },
      getShortBlocklist: () => new Set<string>(),
      entryClaims: { tryClaimEntrySymbol: () => true, releaseEntrySymbol: () => {} },
    ...
    await lane.tick(); // freeze only the initial non-MOM36 candidate set
    store.arm(new Date(now.value).toISOString());
    ...
    const day = store.getState().days[`NY:${window.date}`]!;
    const watermark = window.rangeCloseTime + 10 * 60_000;
    expect(day.universeSymbols).toEqual(["BASEUSDT", "BORROWUSDT"]);
    expect(day.borrowedUniverseAdmissions?.BORROWUSDT?.poolEvidence?.activeSymbols).toEqual(["BORROWUSDT"]);
    expect(day.symbolStates.BORROWUSDT?.lastProcessedBarOpenTime).toBe(watermark);
    expect(store.getState().signals.filter((row) => row.symbol === "BORROWUSDT")).toEqual([]);
  });

  it("arms and safely exits a new Fade through the Stage 2 MFE floor while leaving Continuation untouched", async () => {
    const window = newYorkDailyRangeWindow(Date.UTC(2026, 7, 26, 12));
    const now = { value: window.rangeCloseTime + 16 * 60_000 };
    const fadeClient = new FakeDailyClient();
    fadeClient.now = now.value;
    const fade = makeLane(fadeClient, now, ["AAAUSDT"], { tryClaimEntrySymbol: () => true, releaseEntrySymbol: () => {} }, "AUTO_ROUTE_NY_V2");
    fade.store.arm(new Date(window.rangeCloseTime + 14 * 60_000).toISOString());
    const fadeSignal = autoRouteSignal({
      symbol: "AAAUSDT", decisionAt: now.value, route: "FADE", direction: "SHORT", breakoutDirection: "UP", breakoutExtreme: 102,
    });
    fade.store.getState().signals.push(fadeSignal);
    await (fade.lane as unknown as { executeFreshSignal(signalArg: DailyRangeSignal): Promise<void> }).executeFreshSignal(fadeSignal);
    const fadeTrade = fade.store.getState().trades[0]!;
    expect(fadeTrade).toMatchObject({ status: "OPEN", entryPolicy: "FADE", fadeMfe: { mfePolicyId: "daily-fade-mfe-50-75-v1" } });
    expect(fade.lane.getStatus()).toMatchObject({
      fadeMfe: { policyId: "daily-fade-mfe-50-75-v1", appliesTo: "NEW_BREAKOUT_FADE_ONLY", activeOpenTrades: 1 },
    });

    now.value += 1;
    fadeClient.now = now.value;
    fade.lane.ingestContractPricePath({ symbol: "AAAUSDT", price: 97, eventTimeMs: now.value, receivedAtMs: now.value, source: "CONTRACT_AGG_TRADE", streamStartedAtMs: now.value - 10 });
    expect(fadeTrade.fadeMfe).toMatchObject({ stage1Armed: true, stage2Armed: true, mfeExitFloorProgress: 0.5, mfeExitFloorPrice: 98 });

    now.value += 1;
    fadeClient.now = now.value;
    fade.lane.ingestContractPricePath({ symbol: "AAAUSDT", price: 98, eventTimeMs: now.value, receivedAtMs: now.value, source: "CONTRACT_AGG_TRADE", streamStartedAtMs: now.value - 11 });
    for (let index = 0; index < 12 && fadeTrade.status !== "CLOSED"; index++) await new Promise((resolve) => setTimeout(resolve, 0));
    expect(fadeTrade).toMatchObject({
      status: "CLOSED",
      exitReason: "FADE_MFE_STAGE2_GIVEBACK_EXIT",
      fadeMfe: { exitAttribution: { terminalOutcome: "MFE_EXIT", highestArmedStage: 2, triggerPrice: 98 } },
    });
    expect(fadeClient.placed.filter((order) => order.reduceOnly)).toHaveLength(1);
    expect(await fadeClient.getOpenAlgoOrders("AAAUSDT")).toHaveLength(0);
    expect(fade.lane.getStatus()).toMatchObject({ fadeMfe: { activeOpenTrades: 0, stage2ExitCount: 1 } });

    const continuationClient = new FakeDailyClient();
    continuationClient.now = now.value;
    const continuation = makeLane(continuationClient, now, ["BBBUSDT"], { tryClaimEntrySymbol: () => true, releaseEntrySymbol: () => {} }, "AUTO_ROUTE_NY_V2");
    continuation.store.arm(new Date(window.rangeCloseTime + 14 * 60_000).toISOString());
    const continuationSignal = autoRouteSignal({
      symbol: "BBBUSDT", decisionAt: now.value, route: "CONTINUATION", direction: "LONG", breakoutDirection: "UP", breakoutExtreme: 103,
    });
    continuation.store.getState().signals.push(continuationSignal);
    await (continuation.lane as unknown as { executeFreshSignal(signalArg: DailyRangeSignal): Promise<void> }).executeFreshSignal(continuationSignal);
    const continuationTrade = continuation.store.getState().trades[0]!;
    continuation.lane.ingestContractPricePath({ symbol: "BBBUSDT", price: 109, eventTimeMs: now.value + 1, receivedAtMs: now.value + 1, source: "CONTRACT_AGG_TRADE", streamStartedAtMs: now.value - 10 });
    continuation.lane.ingestContractPricePath({ symbol: "BBBUSDT", price: 105, eventTimeMs: now.value + 2, receivedAtMs: now.value + 2, source: "CONTRACT_AGG_TRADE", streamStartedAtMs: now.value - 10 });
    await new Promise((resolve) => setTimeout(resolve, 0));
    expect(continuationTrade.fadeMfe ?? null).toBeNull();
    expect(continuationTrade.status).toBe("OPEN");
    expect(continuationClient.placed.filter((order) => order.reduceOnly)).toHaveLength(0);
  });

  it("lets a native TP or SL win the MFE race without a second reduce-only close", async () => {
    const window = newYorkDailyRangeWindow(Date.UTC(2026, 7, 26, 12));
    for (const native of ["tp", "sl"] as const) {
      const now = { value: window.rangeCloseTime + 16 * 60_000 };
      const client = new FakeDailyClient();
      client.now = now.value;
      const subject = makeLane(client, now, ["AAAUSDT"], { tryClaimEntrySymbol: () => true, releaseEntrySymbol: () => {} }, "AUTO_ROUTE_NY_V2");
      subject.store.arm(new Date(window.rangeCloseTime + 14 * 60_000).toISOString());
      const row = autoRouteSignal({ symbol: "AAAUSDT", decisionAt: now.value, route: "FADE", direction: "SHORT", breakoutDirection: "UP", breakoutExtreme: 102 });
      subject.store.getState().signals.push(row);
      await (subject.lane as unknown as { executeFreshSignal(signalArg: DailyRangeSignal): Promise<void> }).executeFreshSignal(row);
      const trade = subject.store.getState().trades[0]!;
      now.value += 1;
      client.now = now.value;
      subject.lane.ingestContractPricePath({ symbol: "AAAUSDT", price: 97, eventTimeMs: now.value, receivedAtMs: now.value, source: "CONTRACT_AGG_TRADE", streamStartedAtMs: now.value - 10 });

      const nativeId = `native-${native}`;
      const algo = client.algos.get(native === "tp" ? trade.takeProfitAlgoOrderId! : trade.stopAlgoOrderId!)!;
      const exitPrice = native === "tp" ? trade.takeProfitPrice! : trade.stopPrice!;
      algo.algoStatus = "FINISHED";
      algo.actualOrderId = nativeId;
      client.algoHistory.set(algo.algoId, { ...algo });
      client.orders.set(nativeId, {
        symbol: trade.symbol, orderId: nativeId, clientOrderId: nativeId, status: "FILLED", type: "MARKET", side: "BUY", reduceOnly: true,
        price: 0, stopPrice: 0, origQty: trade.entryQty!, executedQty: trade.entryQty!, avgPrice: exitPrice, updateTime: now.value,
      });
      client.fills.push({ symbol: trade.symbol, orderId: nativeId, price: exitPrice, qty: trade.entryQty!, realizedPnl: native === "tp" ? 1 : -1, commission: -0.01, commissionAsset: "USDT", time: now.value });
      client.positions.set(trade.symbol, { symbol: trade.symbol, positionAmt: 0, entryPrice: 100, markPrice: exitPrice, liquidationPrice: 0, unRealizedProfit: 0, leverage: 1, marginType: "CROSSED" });

      now.value += 1;
      client.now = now.value;
      subject.lane.ingestContractPricePath({ symbol: "AAAUSDT", price: 98, eventTimeMs: now.value, receivedAtMs: now.value, source: "CONTRACT_AGG_TRADE", streamStartedAtMs: now.value - 11 });
      for (let index = 0; index < 12 && trade.status !== "CLOSED"; index++) await new Promise((resolve) => setTimeout(resolve, 0));
      expect(trade.exitReason).toBe(native === "tp" ? "TAKE_PROFIT" : "STOP_LOSS");
      expect(client.placed.filter((order) => order.reduceOnly)).toHaveLength(0);
      expect(trade.fadeMfe?.exitAttribution ?? null).toBeNull();
    }
  });

  it("persists an armed Fade MFE state through restart and fails closed on a stale path event", async () => {
    const window = newYorkDailyRangeWindow(Date.UTC(2026, 7, 26, 12));
    const now = { value: window.rangeCloseTime + 16 * 60_000 };
    const client = new FakeDailyClient();
    client.now = now.value;
    ...
    now.value += 1;
    client.now = now.value;
    first.lane.ingestContractPricePath({ symbol: "AAAUSDT", price: 97, eventTimeMs: now.value, receivedAtMs: now.value, source: "CONTRACT_AGG_TRADE", streamStartedAtMs: now.value - 10 });
    const before = first.store.getState().trades[0]!.fadeMfe!;
    expect(before).toMatchObject({ stage1Armed: true, stage2Armed: true, mfeExitFloorProgress: 0.5 });
    ...
    restarted.ingestContractPricePath({ symbol: "AAAUSDT", price: 98, eventTimeMs: now.value, receivedAtMs: now.value - 6_000, source: "CONTRACT_AGG_TRADE", streamStartedAtMs: now.value - 10 });
    const after = new DailyRangeLaneStore(first.dir, "state.json", now.value).getState().trades[0]!;
  ...
  it("resumes a durable Fade MFE exit intent after restart without waiting for another price event", async () => {
    ...
    first.store.getState().signals.push(row);
    await (first.lane as unknown as { executeFreshSignal(signalArg: DailyRangeSignal): Promise<void> }).executeFreshSignal(row);
    const trade = first.store.getState().trades[0]!;
    const policy = trade.fadeMfe!;
    policy.stage1Armed = true;
    policy.stage2Armed = true;
    policy.peakMfeProgress = 0.8;
    policy.peakMfePrice = 96.8;
    policy.peakMfeAt = new Date(now.value).toISOString();
    policy.mfeExitFloorProgress = 0.5;
    policy.mfeExitFloorPrice = 98;
    policy.mfeExitIntentAt = new Date(now.value).toISOString();
    policy.mfeExitIntentReason = "FADE_MFE_STAGE2_GIVEBACK_EXIT";
    policy.exitAttribution = {
      ...
      triggerPrice: 98,
      triggerAt: policy.mfeExitIntentAt,
      actualExitFill: null,
    ...
    expect(after).toMatchObject({
      status: "CLOSED",
      exitReason: "FADE_MFE_STAGE2_GIVEBACK_EXIT",
      fadeMfe: { exitAttribution: { terminalOutcome: "MFE_EXIT" } },
    });
    expect(client.placed.filter((order) => order.reduceOnly)).toHaveLength(1);
  });
});
