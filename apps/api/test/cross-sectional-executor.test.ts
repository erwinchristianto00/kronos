...
function makeExecutor(opts: { client?: FakeExecClient; allowed?: boolean; laneWeightPct?: number; rawLaneWeightPct?: number; cortexRealAttribution?: CortexRealAttributionStore; laneId?: string; signalMs?: number; dailyMaxLossUsd?: number; maxOpenBaskets?: number; entryHealthAllowed?: boolean; entryHealthReason?: string | null; siblingOpenLegs?: () => Array<{ symbol: string; side: "LONG" | "SHORT"; qty: number }>; existingNotionalForSymbol?: (symbol: string) => number; maxNotionalPerSymbolAcrossLanes?: number; respectSignalRiskGeometry?: boolean;
  ...
  const executor = new CrossSectionalExecutor({
    ...
    ...(opts.warmPublicQuotes ? { warmPublicQuotes: opts.warmPublicQuotes } : {}),
...
describe("cross-sectional executor (basket execution, testnet-first)", () => {
  ...
  it("[USD-M QUOTE BATCH] warms the complete entry plan once and never fans out to late per-symbol reads", async () => {
    const client = new FakeExecClient();
    const cache = new Map<string, { bid: number; ask: number; mid: number; atMs: number; venue: string }>();
    const warmOne = vi.fn(async () => {
      throw new Error("per-symbol quote warm must not run when a batch warmer is wired");
    });
    const warmBatch = vi.fn(async (symbols: readonly string[]) => {
      for (const symbol of symbols) {
        const mid = symbol === "SOLUSDT" ? 100 : 0.1;
        cache.set(symbol, {
          bid: mid * 0.999,
          ask: mid * 1.001,
          mid,
    ...
    const { executor, store } = makeExecutor({
      ...
      signalMs: NOW_MS - 5 * 60_000,
      requireExecutionVenueQuote: true,
      warmPublicQuote: warmOne,
      warmPublicQuotes: warmBatch,
      readPublicQuote: (symbol) => cache.get(symbol) ?? null,
    });

    await executor.tick();

    expect(warmBatch).toHaveBeenCalledTimes(1);
    expect(warmBatch).toHaveBeenCalledWith(["SOLUSDT", "DOGEUSDT"]);
    expect(warmOne).not.toHaveBeenCalled();
    expect(store.getState().baskets[0]).toMatchObject({ status: "COMPLETE" });
    expect(client.placed.map((order) => order.symbol).sort()).toEqual(["DOGEUSDT", "SOLUSDT"]);
  });

  it("[USD-M QUOTE BATCH] still aborts before any order if a batch leaves one leg without a two-sided book", async () => {
    const client = new FakeExecClient();
    const cache = new Map<string, { bid: number; ask: number; mid: number; atMs: number; venue: string }>();
    const warmOne = vi.fn(async () => null);
    const warmBatch = vi.fn(async () => {
      cache.set("SOLUSDT", {
        bid: 99.99,
        ask: 100.01,
        mid: 100,
