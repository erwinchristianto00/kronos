...
/**
 * The causal feature snapshot uses the 60 completed 5m bars ending at C2.
 * Keep this explicit so the formation pass can fetch exactly the same history
 * once and hand it to research instead of re-reading every pool symbol.
 */
const RESEARCH_HISTORY_BARS = 60;
...
export type DailyRangeExecClient = Pick<
  ...
  | "getIncomeHistory"
> & {
  /** One same-venue BBO snapshot for a whole allocation cohort. Optional only
   * for older fixtures/clients; production uses it to avoid per-symbol quote
   * fan-out at a completed-candle boundary. */
  getExecutionBookTickers?: BinanceFuturesPrivateClient["getExecutionBookTickers"];
  /** Testnet-only observability gate: a known transport cooldown must not be
   * retried by the lane's 30-second scheduler. */
  getRateLimitStatus?: () => { coolingDown: boolean; retryAt: string | null };
};
...
export interface DailyRangeAcceptanceLaneOptions {
  ...
  /**
   * Read-only market-candle source.  Execution, positions, orders and
   * exchange-native brackets always remain on `client`; this optional reader
   * exists so Testnet can use the durable public USD-M market-data transport
   * without consuming the deliberately slow Testnet account-read lease.
   */
  marketKlines?: DailyRangeExecClient["getKlines"];
...
/**
 * Returns a reusable formation snapshot only when it exactly spans the same
 * completed 5m bars the research query would have asked Binance for. A short,
 * gapped, or later snapshot is deliberately rejected so recovery uses the
 * explicit REST path rather than fabricating causal feature data.
 */
function completeResearchHistory(
  rows: readonly DailyRangeCandle[],
  startTime: number,
  endTime: number,
): DailyRangeCandle[] | null {
  const firstOpen = Math.ceil(startTime / FIVE_MIN_MS) * FIVE_MIN_MS;
  const lastOpen = Math.floor((endTime - (FIVE_MIN_MS - 1)) / FIVE_MIN_MS) * FIVE_MIN_MS;
  if (!Number.isFinite(firstOpen) || !Number.isFinite(lastOpen) || lastOpen < firstOpen) return null;
  const expectedCount = Math.floor((lastOpen - firstOpen) / FIVE_MIN_MS) + 1;
  if (expectedCount !== RESEARCH_HISTORY_BARS) return null;
  const scoped = rows
    .filter((row) => row.openTime >= firstOpen && row.openTime <= lastOpen && row.closeTime <= endTime)
    .sort((left, right) => left.openTime - right.openTime);
  if (
    scoped.length !== expectedCount
    || scoped[0]?.openTime !== firstOpen
    || scoped.at(-1)?.openTime !== lastOpen
    || !contiguousFiveMinuteBars(scoped)
    || scoped.some((row) => row.closeTime !== row.openTime + FIVE_MIN_MS - 1)
  ) return null;
  return scoped;
}
...
export class DailyRangeAcceptanceLane {
  ...
  private readonly marketKlines: DailyRangeExecClient["getKlines"];
  ...
  /**
   * Counterfactual maturation is research-only.  It can require a long tail of
   * 1m history reads, so it must never keep the operational tick (native
   * bracket reconciliation, range-reentry exit, or current 5m formation)
   * waiting behind a deliberately conservative Testnet transport queue.
   */
  private backgroundResearchMaturationRunning = false;
  ...
  constructor(opts: DailyRangeAcceptanceLaneOptions) {
    ...
    // Preserve the execution client as the default for Mainnet and every
    // existing fixture.  Testnet may supply a public, production-USD-M candle
    // reader so a scan of many symbols cannot starve account reconciliation
    // behind the host-wide 30-second Testnet read lease.
    this.marketKlines = opts.marketKlines ?? opts.client.getKlines.bind(opts.client);
  ...
  */
  ...
  getStatus(): Record<string, unknown> {
    ...
    const newEntryReason = !newEntriesEnabled
      ...
      : null;
    const structuralContinuationMode = this.autoRouteEntryMode === "CONTINUATION_ACCEPTANCE_EXPANSION";
    return {
      ok: true,
  ...
  async tick(): Promise<void> {
    ...
    try {
      // Testnet's 418 circuit is already a durable, host-wide fact. Repeating
      // startup reconciliation every 30 seconds cannot make it recover and
      // used to overwrite a deliberate operator disarm with a synthetic
      // "startup failed" reason. Stay entirely local until the transport says
      // a physical read is permitted again.
      if (this.environment === "testnet" && this.client.getRateLimitStatus?.().coolingDown === true) {
        if (isRateLimitTransportMessage(state.runtime.lastError)) state.runtime.lastError = null;
        this.store.save();
        return;
      }
      // `reconcileOnStartup()` already performs the exact owned-position and
      // native-algo reconciliation below.  Repeating it in the same cold-start
      // tick doubled Testnet's private reads, then each request waited behind
      // the host-wide 30-second safety lease.  Treat its successful result as
      // this tick's reconciliation; later ticks retain the normal refresh.
      let exchangeReconciled = false;
      if (!this.startupReconciled) {
        await this.reconcileOnStartup();
        exchangeReconciled = true;
      } else {
        exchangeReconciled = await this.reconcileOpenTrades();
      }
      if (exchangeReconciled) {
        ...
        // A fresh owned-position reconciliation proves that a previous
        // transport cooldown is historical.  Clear that Testnet-only UI
        // residue without touching an operator/risk/strategy error.
        if (this.environment === "testnet" && isRateLimitTransportMessage(state.runtime.lastError)) {
      ...
      if (!policyReadyForEntries) {
        this.scheduleBackgroundResearchMaturation();
        this.store.save();
        return;
      }
  ...
  /**
   * Research outcomes remain durable and eventually complete, but they have no
   * authority over a live trade.  Run them singly in the background so a slow
   * public 1m history read cannot make `ticking` stay true and suppress all
   * subsequent operational ticks.
   */
  private scheduleBackgroundResearchMaturation(): void {
    if (this.backgroundResearchMaturationRunning) return;
    this.backgroundResearchMaturationRunning = true;
    void (async () => {
      try {
        ...
        // Research evidence is explicitly fail-closed/pending.  It must not
        // surface as an operational lane failure or block native protection.
        console.warn(`[daily-range-lane] BACKGROUND_RESEARCH_MATURATION_FAILED ${error instanceof Error ? error.message : String(error)}`);
      } finally {
        this.backgroundResearchMaturationRunning = false;
      }
    })();
  }
  ...
  private async reconcileOnStartup(): Promise<void> {
    const state = this.store.getState();
    const recoverTestnetTransportCooldown = this.environment === "testnet"
      && state.control.mode === "DISARMED"
      && /^startup reconciliation failed: rate limited \(HTTP (?:418|429)\); transport cooldown until .+$/i
        .test(state.control.disarmReason ?? "");
    try {
      const manualClosePending = state.trades.some((trade) =>
        trade.status === "EXIT_RECONCILING"
        && trade.exitReason === "MANUAL_CLOSE"
        && !trade.exitOrderId,
      );
      if (manualClosePending) {
        // A durable manual close is a risk-reducing recovery action, not an
        // entry.  Do it before startup capability checks such as hedge-mode
        // verification: those checks can wait behind Testnet's intentionally
        // slow shared GET lease, whereas this branch only proves the already
        // persisted exact exit identity.  The lane remains disarmed and
        // startupReconciled remains false until normal account reconciliation
        // has completed, so this shortcut cannot create a new entry.
      ...
      await this.reconcileOpenTrades();
      // A Testnet-only transport cooldown is neither an operator disarm nor a
      // strategy/risk failure.  Once the exact same startup reconciliation
      // succeeds, restore the prior entry authority at a fresh candle boundary
      // so an old C1/C2 pair can never be entered late.  All other disarm
      // reasons remain sticky and still require the normal explicit arm path.
      if (recoverTestnetTransportCooldown) {
        const priorReason = state.control.disarmReason;
        this.resetTodayDetectionAtArm();
        this.store.arm(iso(this.nowMs()));
        // The successful reconciliation supersedes the stale transient error
        // that put the Testnet lane into cooldown recovery.  Keep unrelated
        // operator/risk errors intact by clearing it only on this exact path.
        state.runtime.lastError = null;
        console.log(`[daily-range-lane] TESTNET_AUTO_REARM_AFTER_TRANSPORT_COOLDOWN prior=${priorReason}`);
      }
      ...
      const priorDisarm = state.control.disarmReason ?? "";
      const preserveManualTestnetDisarm =
        this.environment === "testnet" &&
        isRateLimitedTransportFailure(error) &&
        state.control.mode === "DISARMED" &&
        !/^startup reconciliation failed: rate limited \(HTTP (?:418|429)\); transport cooldown until .+$/i.test(priorDisarm);
      if (!preserveManualTestnetDisarm) {
        this.store.disarm(iso(this.nowMs()), `startup reconciliation failed: ${message}`);
      } else {
        // Persist the transient diagnostic without changing who owns the
        // disarm decision. A later successful reconciliation must therefore
        // not auto-arm this lane.
        this.store.save();
      }
      throw error;
    }
  ...
  private async buildDailyReferenceLevel(
    ...
    try {
      ...
      if (this.isAutoRoute()) {
        ...
        const rows = await this.marketKlines(symbol, "5m", {
        ...
        const candles = await this.marketKlines(symbol, "4h", {
          startTime: window.rangeOpenTime,
          endTime: window.rangeCloseTime - 1,
          limit: 3,
        });
  ...
  private async ensureTodayRange(): Promise<boolean> {
    ...
    const existingPolicyMatches = existingDay !== null
      && existingDay.strategyVersion === this.strategyVersion
      && existingDay.strategyMode === this.strategyMode
      && (!this.isAutoRouteV3() || existingDay.autoRouteEntryMode === this.autoRouteEntryMode);
        ...
        try {
          ...
          if (this.isAutoRoute()) {
            ...
            const rows = await this.marketKlines(symbol, "5m", {
              startTime: window.rangeOpenTime,
              endTime: window.rangeCloseTime - 1,
              // 00:00-04:00 New York contains exactly 48 finished 5m bars.
              // Requesting 100 raises Binance kline request weight for no data.
              limit: (window.rangeCloseTime - window.rangeOpenTime) / FIVE_MIN_MS,
            });
            const exact = rows.map(asDailyCandle)
              .filter((candle) => candle.openTime >= window.rangeOpenTime && candle.closeTime < window.rangeCloseTime)
              .sort((left, right) => left.openTime - right.openTime);
  ...
  private async processCompletedBars(): Promise<void> {
    ...
    // A completed formation pass already needs every pool symbol's current
    // 5m history.  Retain the last causal 60 bars from that exact request for
    // the V3 feature snapshot below.  Before this cache, a 42-symbol universe
    // fetched the same candle window once to form signals and again for
    // research, creating the Testnet REST burst behind recurring HTTP 418s.
    // This is in-memory only: a restart deliberately falls back to an explicit
    // REST reconstruction rather than inventing missing historical evidence.
    const completedResearchHistories = new Map<string, DailyRangeCandle[]>();
    const researchHistoryStart = latestCompletedOpen - (RESEARCH_HISTORY_BARS - 1) * FIVE_MIN_MS;
    for (const [symbol, level] of Object.entries(day.levels)) {
      ...
      let completed: DailyRangeCandle[];
      try {
        const fetchStart = Math.min(initialOpen, researchHistoryStart);
        const requestedBars = Math.floor((latestCompletedOpen - fetchStart) / FIVE_MIN_MS) + 1;
        const rows = await this.marketKlines(symbol, "5m", {
          startTime: fetchStart,
          endTime: latestCompletedOpen + FIVE_MIN_MS - 1,
          // The normal scan needs the causal 60-bar feature window, not a
          // 500-bar request (whose Binance kline weight is materially higher).
          // A genuine restart catch-up keeps the historical 500-bar ceiling.
          limit: Math.min(500, Math.max(RESEARCH_HISTORY_BARS, requestedBars)),
        });
        completed = rows
          .map(asDailyCandle)
          .filter((bar) => bar.openTime >= fetchStart && bar.openTime <= latestCompletedOpen && bar.closeTime < now)
          .sort((a, b) => a.openTime - b.openTime);
        completedResearchHistories.set(
          symbol,
          completed.filter((bar) => bar.openTime >= researchHistoryStart),
        );
        candles = completed.filter((bar) => bar.openTime >= initialOpen);
      } catch (error) {
      ...
      for (const candle of candles) {
        ...
        // The C2 decision can use only rows whose own 5m close was already
        // known at this point.  Pass that causal slice through to the new
        // structural-continuation snapshot; never re-read a later chart.
        const causalCandles = completed
          .filter((row) => row.openTime <= candle.openTime)
          .slice(-RESEARCH_HISTORY_BARS);
        const emitted = this.applyCandle(day, level, symbolState, candle, causalCandles);
    ...
    if (this.isAutoRoute()) {
      await this.capturePendingResearchSnapshots(day, "FORWARD_BEFORE_ALLOCATION", completedResearchHistories);
    }
    await this.finalizePendingSignalBatches(day);
    await this.capturePendingResearchSnapshots(day, "RECOVERY_AFTER_ALLOCATION", completedResearchHistories);
    this.scheduleBackgroundResearchMaturation();
    this.syncSignalCohortDecisions();
  }

  /**
   * Replays only completed, contiguous 5m candles for frozen policies that
   * explicitly opt into a thesis invalidation. It has no entry path and never
   * interprets websocket ticks or intra-bar wicks as a thesis failure.
   */
  private async processRouteSpecificThesisInvalidations(): Promise<void> {
    ...
    for (const trade of active) {
      ...
      while (nextOpen <= latestCompletedOpen && trade.status === "OPEN" && !trade.thesisInvalidation) {
        ...
        try {
          rows = (await this.marketKlines(trade.symbol, "5m", {
            startTime: nextOpen,
  ...
  private async matureThesisInvalidationCounterfactuals(): Promise<void> {
    ...
    await Promise.all(pending.map(async (trade) => {
      ...
      try {
        const rows = await this.marketKlines(trade.symbol, "1m", {
          startTime,
  ...
  private applyCandle(
    ...
    causalCandles: readonly DailyRangeCandle[] = [],
  ...
  private async captureSignalTimeMarketQuality(
    ...
    // A cohort may contain many valid signals. One USD-M batch response gives
    // each one a contemporaneous book; individual quote GETs were serialized
    // behind the Testnet account-read lease and produced 7x BBO_STALE.
    // If the batch itself fails, record an honest absence for the whole cohort
    // rather than retrying per symbol and recreating an IP-level burst.
    let batchBooks: Map<string, FuturesExecutionBookTicker> | null = null;
    if (this.client.getExecutionBookTickers) {
      try {
        batchBooks = await this.client.getExecutionBookTickers([...new Set(missing.map((signal) => signal.symbol))]);
      } catch {
        batchBooks = new Map();
      }
    }
    await Promise.all(missing.map(async (signal) => {
      ...
      try {
        const book = batchBooks === null
          ? await this.client.getBookTicker(signal.symbol)
          : batchBooks.get(signal.symbol) ?? null;
        if (book) {
          bestBid = book.bid;
          bestAsk = book.ask;
  ...
  /**
   * Re-arming creates a fresh observation boundary. A V1 candidate generated
  ...
  private async causalAtr4hForDecision(symbol: string, decisionAtMs: number): Promise<DailyRangeAtr4hFeature | null> {
    try {
      const rows = await this.marketKlines(symbol, "4h", {
  ...
  private async capturePendingResearchSnapshots(
    ...
    completedResearchHistories: ReadonlyMap<string, readonly DailyRangeCandle[]> | null = null,
  ): Promise<void> {
    const batches = this.store.getState().signalCohorts
    ...
    for (const batch of batches) {
      ...
      const candleStart = candleEnd - RESEARCH_HISTORY_BARS * FIVE_MIN_MS;
      ...
      for (const symbol of universeSymbols) {
        const cached = completedResearchHistories
          ? completeResearchHistory(completedResearchHistories.get(symbol) ?? [], candleStart, candleEnd)
          : null;
        if (cached) histories.set(symbol, cached);
        else symbolsNeedingFetch.push(symbol);
      }
      // A cache is used only when it is complete and exactly covers the
      // candidate's finished C2 window.  Older/recovered cohorts therefore
      // retain the old explicit reconstruction path instead of borrowing a
      // later bar and contaminating causal evidence.
      await Promise.all(symbolsNeedingFetch.map(async (symbol) => {
        try {
          const rows = await this.marketKlines(symbol, "5m", {
            startTime: candleStart,
            endTime: candleEnd,
            limit: RESEARCH_HISTORY_BARS,
          });
  ...
  private async readReferenceFourHour(signal: DailyRangeSignal, day: DailyRangeDayState): Promise<DailyRangeCandle[]> {
    ...
    try {
      if (isAutoRouteStrategyVersion(signal.strategyVersion)) {
        ...
        const rows = await this.marketKlines(signal.symbol, "5m", {
          startTime,
          endTime: endTime - 1,
          limit: Math.max(1, Math.floor((endTime - startTime) / FIVE_MIN_MS)),
        });
        const candles = rows.map(asDailyCandle)
      ...
      const rows = await this.marketKlines(signal.symbol, "4h", {
  ...
  private async matureCounterfactualOutcomes(): Promise<void> {
    ...
    await Promise.all(pending.map(async (signal) => {
      ...
      try {
        ...
        const rows = await this.marketKlines(signal.symbol, "1m", {
          startTime,
  ...
  private async readVisiblePosition(symbol: string, maxAttempts = CONFIRM_RETRIES): Promise<FuturesPosition | null> {
    const attempts = Math.max(1, Math.min(CONFIRM_RETRIES, Math.floor(maxAttempts)));
    for (let attempt = 0; attempt < attempts; attempt++) {
      ...
      if (attempt + 1 < attempts && this.confirmRetryMs > 0) {
  ...
  private async reconcileOpenTrades(): Promise<boolean> {
    ...
    // A manual close is a durable, operator-requested risk-reduction action.
    // Do it before starting the normal account-wide reconciliation fan-out:
    // that fan-out otherwise consumes the Testnet host-wide read slots and can
    // make the close wait behind its own observability work.  Native brackets
    // remain live until emergencyFlatten proves the exchange position flat, so
    // temporarily deferring routine reconciliation cannot leave another trade
    // naked.  The persisted client id makes this safe across a process restart:
    // if Binance already accepted the exit, the retry resolves that exact order
    // rather than submitting another close.
    const manualClosePending = active.filter((trade) =>
      trade.exitReason === "MANUAL_CLOSE" && !trade.exitOrderId,
    );
    if (manualClosePending.length > 0) {
      for (const trade of manualClosePending) {
        await this.emergencyFlatten(trade, "CLOSED", "MANUAL_CLOSE");
      }
      return manualClosePending.every((trade) => isTerminalTradeStatus(trade.status));
    }
  ...
  private async finalizeFlatTrade(trade: DailyRangeTrade, openAlgos: FuturesAlgoOrder[]): Promise<void> {
    ...
    try {
      ...
      // A manual market close may have filled before its HTTP response timed
      // out. Normal flat-account reconciliation must preserve that exact owner
      // identity too, rather than only looking at native algo brackets and
      // incorrectly leaving a safely-flat manual exit pending.
      if (!exitOrderId && exitReason === "MANUAL_CLOSE" && trade.exitClientOrderId) {
        try {
          const manual = await this.client.queryOrderByClientId(trade.symbol, trade.exitClientOrderId);
          if (orderStatusFilled(manual)) exitOrderId = manual.orderId;
        } catch {
          // The original close might not have reached Binance. Fall through to
          // native-bracket proof; do not invent an exit order or cancel a
          // still-needed protective sibling.
        }
      }
      if (!exitOrderId) {
  ...
  private async emergencyFlatten(
    ...
    try {
      ...
      // Persist the idempotent close intent before the first exchange read.
      // A restart or a client timeout can now resume the same reduce-only
      // order identity; it must never turn into a second market close.
      // A persisted client id means a previous close call has already crossed
      // its durable timeout boundary. It must be reconciled, not treated as a
      // fresh close whose position visibility deserves four paced Testnet
      // polls. One current position read is enough to decide whether to query
      // that exact immutable exit identity; this avoids a 90s+ recovery queue
      // while preserving the full confirmation loop for a brand-new close.
      const resumingPersistedExit = Boolean(trade.exitClientOrderId);
      ...
      const position = await this.readVisiblePosition(
        trade.symbol,
        resumingPersistedExit ? 1 : CONFIRM_RETRIES,
      );
      if (!position) {
        // A reduce-only MARKET order can flatten the exchange before its POST
        // response reaches this process (or before an operator's HTTP request
        // times out).  Resolve the durable client id before treating flatness
        // as unknown; otherwise the position is safe but the ledger and owned
        // protective siblings remain stranded in EXIT_RECONCILING.
        if (trade.exitClientOrderId) {
          try {
            const confirmed = await this.client.queryOrderByClientId(trade.symbol, trade.exitClientOrderId);
            if (orderStatusFilled(confirmed)) {
              ...
              console.log(`[daily-range-lane] ${reason} reconciled trade=${trade.tradeId} symbol=${trade.symbol} exit=${trade.exitPrice}`);
              return;
            }
          } catch {
            // The exact client id may not yet be queryable. Keep both native
            // brackets live and let the durable reconciliation retry later.
          }
        }
      ...
      try {
        // Exit cost telemetry must not make a reduce-only close wait behind
        // a background candle backlog. The quote is optional; the close is not.
        const book = await this.client.getBookTicker(trade.symbol, "EXECUTION");
