...
export class CrossSectionalExecutor {
  ...
  getStatus(): {
    ...
    const formationFresh = formationAgeMs !== null && formationAgeMs >= 0 && formationAgeMs <= signalMaxAgeMs;
    ...
    const signalObservability: CrossSectionalSignalObservability = {
      state: !isDynamicMom36ShockVersion(currentPolicyFingerprint.strategy.strategyVersion)
        ? "NOT_DYNAMIC_MOM36"
  ...
  private async placeRemainingLegsLocked(basket: ExecutorBasket, startIndex: number): Promise<void> {
    ...
    for (let i = startIndex; i < plan.length; i++) {
      ...
      try {
        ...
        const resolvedEntry = await this.resolveFillPrice(
          planned.symbol,
          order.orderId,
          order.avgPrice,
          planned.refPrice,
