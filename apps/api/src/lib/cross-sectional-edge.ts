...
function finalPlanAllocationIsExact(plan: DynamicMom36FinalFormationPlan): boolean {
  return plan.selectionInsufficientReason === null &&
    ...
    plan.finalAllocation.longCount + plan.finalAllocation.shortCount === 6 &&
    plan.selectedLongs.length === plan.finalAllocation.longCount &&
    plan.selectedShorts.length === plan.finalAllocation.shortCount &&
    plan.selectedLongs.length + plan.selectedShorts.length === 6;
}
...
function finalPlanRespectsClusterCap(plan: DynamicMom36FinalFormationPlan, maxPerCluster: number): boolean {
  ...
  const sideRespectsCap = (rows: readonly DynamicMom36RankedSymbol[]): boolean => {
    ...
    for (const row of rows) {
      const cluster = clusterOf(row.symbol);
      if (isMajorCluster(cluster)) continue;
      const next = (counts.get(cluster) ?? 0) + 1;
      if (next > maxPerCluster) return false;
      counts.set(cluster, next);
...
function evaluateDynamicMom36FinalAdmission(
  ...
  let passed = true;
  if (!exactPlan) {
    reason = plan.selectionGuardReason ?? "ADMISSION_FINAL_ALLOCATION_INFEASIBLE";
    passed = false;
  } else if (!strictLegsValid) {
    reason = "ADMISSION_SELECTED_LEG_INVALID";
    passed = false;
...
export function validateDynamicMom36FormationAdmissionParity(
  ...
  if (
    ...
    snapshot.selectedLongs.length + snapshot.selectedShorts.length !== 6
  ) {
  ...
  if (!snapshot.admission.passed || snapshot.admission.reason !== "ADMISSION_PASSED") {
...
export function evaluateDynamicMom36Formation(input: DynamicMom36FormationInput): DynamicMom36FormationEvaluation {
  ...
  // Formation determines the only executable allocation and occupants.  Admission is strictly
  // downstream: it may approve/reject this immutable plan, but it may never reconstruct a
  // balanced 3L/3S probe or silently select a different basket.
  const plan = buildDynamicMom36FinalFormationPlan(input, strategyVersion, formation);
  const admission = isDynamicMom36FinalAllocationAdmissionVersion(strategyVersion)
    ? evaluateDynamicMom36FinalAdmission(input, plan)
    : evaluateLegacyDynamicMom36Admission(input, plan);
  const noEntryReason = noEntryReasonForDynamicFormation(formation, admission);
