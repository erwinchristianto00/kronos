  ...
  if (liveConfig.enabled && liveConfig.configErrors.length === 0 && liveConfig.env) {
    ...
    if (!isTest && (liveConfig.env === "testnet" || liveConfig.env === "mainnet")) {
      ...
      dailyRangeLane = new DailyRangeAcceptanceLane({
        ...
        // V3 retains immutable legacy/V2 trades but makes the new route
        // explicit: Live takes only the first failed-breakout re-entry; Testnet
        // retains continuation and enters on the first completed breakout.
        strategyMode: "AUTO_ROUTE_NY_V3",
        autoRouteEntryMode: liveConfig.env === "mainnet"
